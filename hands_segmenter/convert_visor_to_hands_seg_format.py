...
if __name__ == '__main__':
    ...
    for split in args.split:
        ...
        epick_visor_img_dir = f'{epick_visor_coco_dir}/{split}/img'
        epick_visor_mask_dir = f'{epick_visor_coco_dir}/{split}/mask'
        ...
        for json_path in tqdm(json_ls):
            ...
            with open(json_path, 'r') as f:
                ...
                for item in v_annotations:
                    ...
                    print()
                    if not os.path.exists(os.path.join(img_dir, item['image']['name'])):
                        continue

                    print(item['image']['name'])

                    sample = Image.open( os.path.join(img_dir, item['image']['name'] ))
                    ...
                    for e_idx, entity in enumerate(entities):
                        ...
                        if entity['name'] in ['left hand', 'right hand']: 
                            ...
                            if args.mode in ['handonly', 'active', 'all']:
                                ...
                                for poly in h_masks_coco:
                                    # poly = [[int(cord[0]), int(cord[1])] for cord in  poly]
                                    # print(poly)
                                    ImageDraw.Draw(mask).polygon(poly, outline=255, fill=255)
                                
                                new_mask_path = os.path.join(f'{epick_visor_mask_dir}', img_name)
                                print(new_mask_path)
