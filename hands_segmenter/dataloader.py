...
class EpickDataset(Dataset):
    """
        Epick dataset from https://epic-kitchens.github.io/2023
    """
    def __init__(self, data_base_path, partition, image_transform=None, 
                 mask_transform=None, seed=1234):
        super(EpickDataset, self).__init__()
    ...
    def _get_paths(self):
        ...
        # Image paths
        image_names = sorted(os.listdir(os.path.join(self.data_base_path, 'img')))
        image_paths = [os.path.join(self.data_base_path, 'img', f) for f in image_names]
    ...
    def _get_paths(self):
        ...
        with open(os.path.join(self.data_base_path, 'train-val-test-split', split_filename), 'r') as f:
            for l in f.readlines():
                l = l.strip()
                if not os.path.exists(os.path.join(self.data_base_path, 'masks', l.replace('.jpg', '.png'))):
                    continue
...
def get_dataloader(data_base_path, partition, datasets, image_transform=None,
    ...
    if 'epick' in datasets:
        tranform = transforms.ToTensor()
        ds_epick = EpickDataset(
            data_base_path=os.path.join(data_base_path, 'epick_dataset'),
            partition=partition,
    ...
    dl = DataLoader(ds_cat, batch_size=batch_size, pin_memory=pin_memory, num_workers=num_workers, shuffle=shuffle,drop_last=True)
    return dl
